...
import { Placement, Shape, Align } from '../chart-configuration';
import { drawLegendColorItemByCircle, drawLegendColorItemByLine, drawLegendColorItemByRect, drawSvgCheckBox, getMaxText, getTextWidth, getTextHeight } from '../util';
import { ObjectScrollBar } from '../../svg-component/object-scroll-bar';
export var legendItemListByGrouped = function (displayNames, colors) {
    var legendItemList = [];
...
var ChartLegend = /** @class */ (function () {
    ...
    ChartLegend.prototype.parseLegendData = function (seriesList) {
        ...
        seriesList.forEach(function (series) {
            if (series.displayNames && series.displayNames.length) {
                ...
                legendItemListByGrouped(series.displayNames, series.colors).forEach(function (legendItem) {
                    legendItemList.push(legendItem);
                });
            }
            else {
                var label = series.displayName ? series.displayName : series.selector;
                var shape = series.shape ? series.shape : Shape.RECT;
                legendItemList.push({
                    label: label,
                    shape: shape,
                    selected: true,
    ...
    ChartLegend.prototype.init = function () {
        var _this = this;
        var titleTextHeight = getTextHeight(this.configuration.defaultLegendStyle.font.size, this.configuration.defaultLegendStyle.font.family); // 14
        // legend row 한개의 길이
        ...
        if (this.configuration.seriesList.length) {
            ...
            if (this.configuration.seriesList[0].displayNames && this.configuration.seriesList[0].displayNames.length) {
                targetText = getMaxText(this.configuration.seriesList[0].displayNames.map(function (displayName) { return displayName; }));
            }
            else {
                // 일반 시리즈의 경우 범례 설정.
                targetText = getMaxText(this.configuration.seriesList.map(function (series) { return series.displayName || series.selector; }));
            }
        ...
        this.legendItemList.reduce(function (pre, currentItem, currentIndex) {
            ...
            var currentTextWidth = getTextWidth(currentText, _this.configuration.defaultLegendStyle.font.size, _this.configuration.defaultLegendStyle.font.family);
    ...
    ChartLegend.prototype.drawLegend = function (legendGroup) {
        ...
        var legendItemGroup = legendGroup
            ...
            if (_this.configuration.placement === Placement.LEFT || _this.configuration.placement === Placement.RIGHT) {
                if (_this.configuration.placement === Placement.LEFT) {
                    x = _this.legendPadding;
                }
                x += _this.configuration.addTitleWidth;
                y = index * 20 + _this.legendTopBottmPadding;
            }
            if (_this.configuration.placement === Placement.TOP || _this.configuration.placement === Placement.BOTTOM) {
                if (index > 0) {
        ...
        if (this.configuration.isCheckBox) {
            legendItemGroup.each(function (d, index, nodeList) {
                drawSvgCheckBox(select(nodeList[index]), _this.configuration.onLegendCheckBoxClickHandler);
            });
        }
        ...
        legendLabelGroup.each(function (d, i, nodeList) {
            ...
            if (d.shape === Shape.LINE) {
                drawLegendColorItemByLine(select(nodeList[i]), _this.legendItemSize, distictKeys, _this.configuration.colors);
            }
            else if (d.shape === Shape.CIRCLE) {
                drawLegendColorItemByCircle(select(nodeList[i]), _this.legendItemSize, distictKeys, _this.configuration.colors);
            }
            else if (d.shape === Shape.RECT) {
                drawLegendColorItemByRect(select(nodeList[i]), _this.legendItemSize, distictKeys, _this.configuration.colors);
            }
            // if (d.shape === Shape.LINE) {
        ...
        legendLabelGroup
            ...
            .attr('dy', '.35em')
            .attr('transform', function (d, index) {
            var x = d.shape === Shape.NONE ? 0 : _this.legendPadding + _this.legendItemSize.width;
